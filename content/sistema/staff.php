    ...
    <div class="row gy-4" id="medicosContainer">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $nombre_completo = strtoupper($row['nombre'] . " " . $row['paterno'] . " " . $row['materno']);
                ...
                if ($result_horarios->num_rows > 0) {
                    while ($horario = $result_horarios->fetch_assoc()) {
                        ...
                        if (!empty($manana)) {
                            $horarios_por_dia[$dia][] = $manana;
                        }
                        if (!empty($tarde)) {
                            $horarios_por_dia[$dia][] = $tarde;
                        }
                    }
                }
                ...
                foreach ($horarios_por_dia as $dia => $horarios) {
                    if (!empty($horarios)) {
                        $horarios_links = array_map(function ($horario) use ($pronombre, $titulo, $nombre_completo, $dia) {
                            $link = "https://wa.me/51979708672?text=Hola,%20quiero%20una%20cita%20con%20$pronombre%20$titulo%20$nombre_completo%20en%20el%20horario%20$dia%20$horario";
                            return "<a href=\"$link\" class=\"text-decoration-none text-info\" style=\"font-size: 0.85rem;\">$horario</a>";
                        }, $horarios);

                        $dias_horarios[] = [
                            'dia' => $dia,
                            'horarios' => implode(" | ", $horarios_links),
                        ];
                    }
                }
                ...
                $wa_link = "https://wa.me/51979708672?text=Hola,%20deseo%20una%20cita%20con%20$titulo%20$nombre_completo";
                ?>
                <div class="col-md-12 medico" data-nombre="<?php echo $nombre_completo; ?>" data-especialidad="<?php echo $especialidad; ?>">
                    <div class="card card-flush">
                        <div class="row align-items-center">
                            <div class="col-sm-2">
                                <img class="card-img" src="/<?php echo $row['imagen']; ?>" alt="Foto de <?php echo $nombre_completo; ?>">
                            </div>
                            <div class="col-sm-10">
                                <div class="card-body">
                                    <h5 class="card-title text-uppercase text-primary mb-2">
                                        <div class="row">
                                            ...
                                            <div class="col-md-5 d-flex justify-content">
                                                <a href="<?php echo $wa_link; ?>" class="btn btn-naranja d-none d-lg-inline-block rounded-pill me-2" target="_blank">
                                                    <i class="fab fa-whatsapp"></i> Solicitar cita
                                                </a>
                                                <a href="<?php echo $url; ?>" class="btn btn-primary d-none d-lg-inline-block rounded-pill">
                                                    Ver más
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-5">
                                            <?php foreach ($column1 as $h) { ?>
                                                <p class="mb-1 d-flex align-items-center">
                                                    <strong class="text-end me-2" style="font-size: 0.85rem; width: 40%;"><?php echo $h['dia']; ?>:</strong>
